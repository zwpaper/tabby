...
export async function validateTextFile(filePath: string): Promise<void> {
  if (!(await isPlainTextFile(filePath))) {
    throw new Error("File is not plain text");
  }
}
...
export async function isPlainTextFile(filePath: string): Promise<boolean> {
  try {
    ...
    const bufferData = buffer.slice(0, bytesRead);
    ...
    const maxBytesToInspect = Math.min(bytesRead, BUFFER_MAX_LEN);

    for (let i = 0; i < maxBytesToInspect; i++) {
    ...
    // Handle file system errors (e.g., file not found, permissions)
    console.error(`Error checking file ${filePath}:`, error);
    return false; // Or re-throw the error, depending on desired behavior.
  }
}
