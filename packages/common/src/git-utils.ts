...
/**
 * Checks if a git origin URL is from GitHub
 *
 * @param originUrl - The git origin URL
 * @returns true if the URL is from GitHub
 */
export function isGitHubUrl(originUrl: string): boolean {
  const info = parseGitOriginUrl(originUrl);
  return info?.platform === "github";
}
 ...
 */
export function getGitPlatformIcon(platform: GitPlatform) {
  switch (platform) {
    case "github":
      return "github";
    case "gitlab":
