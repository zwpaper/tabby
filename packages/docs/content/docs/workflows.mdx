...
You can use a single line prefixed with `!` to execute a command. The output of the command will be embedded into the prompt sent to the model.

```markdown
!\`git diff --cached\`

Please review the staged changes and suggest improvements.
```
