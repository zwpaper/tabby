...
export const { tool: searchFiles, execute: executeSearchFiles } =
  defineClientTool({
    description:
      ...
      matches: z
        .array(
          z.object({
            ...
            context: z
              .string()
              .describe("The surrounding context of the match."),
          }),
        )
        .describe("List of matches found in the search."),
    }),
    execute: async ({ path, regex, filePattern }, { abortSignal }) => {
      ...
      if (filePattern) {
        ...
        command += `--glob '${filePattern.replace(/'/g, "''")}' `;
      }
