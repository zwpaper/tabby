...
interface RipgrepOutput {
  ...
  data: RipgrepMatchData | any; // Use 'any' for simplicity, or define more types if needed
}
  ...
  try {
    ...
    for (const line of outputLines) {
      try {
        ...
        console.error(`Failed to parse rg JSON output line: ${line}`, parseError);
    ...
    if (error.code && error.code > 1) {
      ...
      throw new Error(`rg command failed with code ${error.code}: ${error.stderr || error.message}`);
    } else if (!error.code && error.stderr) {
