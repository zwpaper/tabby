...
- For editing files, you have access to these tools: applyDiff (for replacing lines in existing files), writeToFile (for creating new files or complete file rewrites).
  * Prefer using applyDiff when changes are small or local.
  * Prefer using writeToFile when multiple places in a file need to be changed to avoid multiple calls to applyDiff.
