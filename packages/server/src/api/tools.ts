...
const ToolDescriptions: Record<string, string> = {
  ...
  batchCall:
    "The `batchCall` tool is an efficiency powerhouse. It allows Pochi to execute multiple tool commands in parallel, significantly speeding up the workflow. Instead of waiting for one operation to finish before starting the next, Pochi can run them all at once. This is especially useful for tasks that require gathering information from multiple sources or making changes to several files.\n\nFor instance, Pochi can use `batchCall` to read three different files, search for a pattern in another set of files, and list the contents of a directory, all in a single step. This reduces the total time it takes to complete your request and allows Pochi to work much faster.",
  executeCommand:
...
const allTools = Object.entries(ClientToolsV5)
  .map(([id]) => ({
    id,
