// Modified from: https://github.com/TabbyML/tabby/blob/493cef3b3229548175de430dbc7f7e4a092ca507/clients/tabby-agent/src/codeCompletion/debouncer.ts
...
export class CompletionDebouncing {
  ...
  async debounce(
    ...
    const config = CodeCompletionConfig.value.debounce;
    if (context.manually) {
    ...
    const adaptiveRate =
      this.rules.adaptiveRate.max -
      (this.rules.adaptiveRate.max - this.rules.adaptiveRate.min) *
        contextScore;
    const expectedLatency = adaptiveRate * this.baseInterval;
    const responseTime = context.estimatedResponseTime ?? 0;
    const delay = clamp(
      this.rules.requestDelay.min,
      this.rules.requestDelay.max,
  ...
  private updateBaseInterval(interval: number) {
    ...
    if (
      ...
      this.rules.baseIntervalSlideWindowAvg.maxSize
    ) {
      this.intervalHistory.shift();
    }
    if (
      this.intervalHistory.length >
      this.rules.baseIntervalSlideWindowAvg.minSize
    ) {
      const avg =
