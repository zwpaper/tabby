...
export class CompletionStatusBarManager {
  ...
  constructor(
    ...
    this.completionConfig.on("updated", (config) => {
      this.logger.trace("Configuration updated, updating status bar", {
  ...
  private updateStatus(configOverride?: { enabled: boolean }): void {
    ...
    // Check if inline completion UI should be shown
    ...
    this.logger.trace("Updating status bar:", {
      hasToken,
      currentStatus: this.currentStatus,
      isEnabled,
      showInlineCompletionUI,
    });

    if (!showInlineCompletionUI) {
    ...
    // Show disabled status if completion is disabled
    if (!isEnabled) {
