...
export class CodeSearchEngine {
  ...
  private chunk(document: TextDocumentSnapshot): Chunk[] {
    const chunks: Chunk[] = [];
    const documentEnd = new vscode.Position(document.lineCount, 0);
    ...
    do {
      ...
      if (positionEnd.isBefore(documentEnd)) {
      ...
      if (positionEnd.isAfter(documentEnd)) {
        // If have passed the end, back to the end
        positionEnd = documentEnd;
      }
      ...
      if (text.trim().length > 0) {
        chunks.push({
          ...
          range: toOffsetRange(chunkRange, document),
          text: text,
      ...
      positionEnd.isBefore(documentEnd)
    );
    return chunks;
  }
   ...
   * Index the document.
   *
   * When invoked multiple times with the same document uri, the old will be removed.
   *
   ...
   */
  async index(document: TextDocumentSnapshot): Promise<void> {
    const documentUriString = document.uri.toString();
    const existingIndex = this.indexed.findIndex(
      (item) => item.document.uri.toString() === documentUriString,
    );
    const existing =
      existingIndex >= 0 ? this.indexed[existingIndex] : undefined;

    const chunks = this.chunk(document);
    if (existing) {
      await this.remove(existing.indexIds);
      this.indexed.splice(existingIndex, 1);
    }
    ...
    while (this.count() > this.config.maxChunks) {
      const toRemove = this.indexed.shift();
      if (toRemove) {
