...
function parseIndentationContext(
  ...
  while (
    referenceLineInPrefixIndex >= 0 &&
    isBlank(prefixLines[referenceLineInPrefixIndex])
  ) {
  ...
  const referenceLineInPrefixIndent = getIndentationLevel(
    referenceLineInPrefix,
  );
  ...
  while (
    referenceLineInCompletionIndex < inputLines.length &&
    isBlank(inputLines[referenceLineInCompletionIndex])
  ) {
  ...
  let referenceLineInCompletionIndent: number;
  if (isCurrentLineInCompletionBlank) {
    referenceLineInCompletionIndent = getIndentationLevel(
      referenceLineInCompletion,
    );
  } else {
    referenceLineInCompletionIndent = getIndentationLevel(
      currentLinePrefix + referenceLineInCompletion,
    );
  }
  ...
  while (
    firstNonBlankLineInSuffix < suffixLines.length &&
    isBlank(suffixLines[firstNonBlankLineInSuffix])
  ) {
    firstNonBlankLineInSuffix++;
  }
  if (firstNonBlankLineInSuffix < suffixLines.length) {
    const firstNonBlankLineInSuffixText =
      suffixLines[firstNonBlankLineInSuffix];
    // allow closing line only if suffix has less indent level
    result.allowClosingLine &&=
      getIndentationLevel(firstNonBlankLineInSuffixText) <
      result.indentLevelLimit;
  }
  return result;
}

export const limitScopeByIndentation: PostprocessFilter = (
  item: string,
  ...
  const { prefixLines, suffixLines, currentLinePrefix, language } =
    baseSegments;

  const inputLinesForDetection = inputLines.map((line, index) => {
    return index === 0 ? currentLinePrefix + line : line;
  });
  const indentContext = parseIndentationContext(
    inputLines,
    inputLinesForDetection,
    baseSegments,
  );
  let index = 1;
  while (index < inputLines.length) {
    ...
    if (indentLevel < indentContext.indentLevelLimit) {
      ...
      if (
        indentContext.allowClosingLine &&
        (language !== "python" || !isBlank(prevLine))
      ) {
        index++;
      }
      break;
