...
export const normalizeIndentation: PostprocessFilter = (
  item: string,
  ...
  if (
    !Array.isArray(lines) ||
    lines.length === 0 ||
    baseSegments.currentLinePrefix.length === 0 ||
    baseSegments.currentLinePrefix.includes("\t")
  ) {
    return item;
  }
  if (!isOnlySpaces(baseSegments.currentLinePrefix)) {
    return item;
  }

  const normalizedLines = [...lines];
  const firstLine = normalizedLines[0];
  const cursorLineSpaces = getLeadingSpaces(baseSegments.currentLinePrefix);
  if (firstLine) {
    const firstLineSpaces = getLeadingSpaces(firstLine);
    // deal with current cursor odd indentation
    if (
      (firstLineSpaces + cursorLineSpaces) % 2 !== 0 &&
      firstLineSpaces % 2 !== 0
    ) {
